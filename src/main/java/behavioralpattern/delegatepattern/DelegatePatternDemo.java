 ...
 *
 * 在源码中的应用：
 * JDK中有一个典型的委派，众所周知JVM在加载类用的是双亲委派模型；一个类加载器在加载类时，
 * 先把这个请求委派给自己的父类加载器去执行，如果父类加载器还存在父类加载器，就继
 * 续向上委派，直到顶层的启动类加载器。如果父类加载器能够完成类加载，就成功返回，如果
 * 父类加载器无法完成加载，那么子加载器才会尝试自己去加载。从定义中可以看到双亲加载
 * 模型一个类加载器加载类时，首先不是自己加载，而是委派给父加载器。
 *
 * 委派模式优缺点
 * 优点：
